...
var customPopover = (function () {
  var init = function init() {
    ...
    $('[rel="popover"]').each(function () {
      var { target, textAlign, anchorClose, callbackFunction, position } =
        $(this).data();
      ...
      var popoverAnchors = document
        .getElementById(target)
        .querySelectorAll('.popover__anchor');
      var htmlContent =
        '<div class="popover__wrapper" rel="' +
        target +
        '">' +
        targetHTML +
        '</div>';
      ...
      if (anchorClose) {
        ...
        $(this).on('shown.bs.popover', function () {
          var anchors = $(
            '.popover__wrapper .content-' + target + ' .popover__anchor',
          );
