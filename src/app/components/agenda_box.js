  ...
  var init = function init() {
    ...
    if ($('#profile-page').length > 0) {
      ...
      function getPreviousMonday() {
        ...
        var mondayFormat =
          mondayUnformatted.getFullYear() +
          '-' +
          pad(mondayUnformatted.getMonth() + 1) +
          '-' +
          pad(mondayUnformatted.getDate());
      ...
      $.getJSON(
        ...
        function (response) {
          var htmlBase =
            '<div class="step-agenda" data-month="' +
            response.month +
            '" data-year="' +
            response.year +
            '">' +
      ...
      $('.next_arrow').on('click', function () {
        var slideIndex = $('.agenda_slider .slick-active').attr(
          'data-slick-index',
        );
      ...
      $(document).on('click', '.available-date', function (event) {
        ...
        $('.modal-agenda_month').html(
          $('.step-agenda.slick-active').attr('data-month'),
        );
        $('.modal-agenda_day').html(
          $(this).children('.agenda_input-hour').attr('data-day'),
        );
        $('.modal-agenda_year').html(
          $('.step-agenda.slick-active').attr('data-year'),
        );
